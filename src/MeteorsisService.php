...
class MeteorsisService extends SMSAbstract implements SMSInterface
{
    //驗證訊息
    ...
    function __construct(){
        $this->functions = config('Meteorsis.functions');
        $this->api_err_code = config('Meteorsis.api_err_code');
        $this->sms_status = config('Meteorsis.sms_status');
        $this->sms_status_err_code = config('Meteorsis.sms_status_err_code');
        $this->arrServiceCountry = config('Meteorsis.service_country');
        $this->maxWords = config('Meteorsis.max_words');

        $this->base_param_table = [
            'username' => config('services.meteorsis.key'),
            'password' => config('services.meteorsis.secret'),
        ];
    ...
    /**
     * 3.1
     * @param \Cuby\Meteorsis\MeteorsisMessage $message
     * @return array
     */
    public function send(MeteorsisMessage $message): array
    {
        $this->base_param_table['senderid'] = $message->senderid;
        $this->base_param_table['recipient'] = implode(',',$message->recipients);
        $this->base_param_table['dos'] = $message->dos;
        $this->base_param_table['langeng'] = $message->encode;
        $this->base_param_table['content'] = $message->content;

        $result = $this->queryServer('send_sms');
        if(array_key_exists('SMSDID',$result)){
            event(new MeteorsisSendSMSEvent('MeteorsisService',$result['SMSDID']));
        }
    ...
    public function getSmsStatus(String $sms_did)
    {
        ...
        if(array_key_exists('STATUS',$result)){
            event(new MeteorsisGetSMSStatusEvent('MeteorsisService',$sms_did,$result['STATUS'], $status_text, $result['ERRORCODE'], $errorcode_text));
        }
        return $result;
    }
    /**
    ...
    public function getAccountBalance(){
        ...
        if(array_key_exists('BALANCE',$result)){
            event(new MeteorsisGetAccountBalanceEvent('MeteorsisService',$result['BALANCE']));
        }
        return $result;
    }
    /**
    ...
    public function getServerQuery(){
        ...
        if(array_key_exists('QUEUE',$result)){
            event(new MeteorsisGetServerQueryEvent('MeteorsisService',$result['QUEUE']));
        }
        return $result;
    }
